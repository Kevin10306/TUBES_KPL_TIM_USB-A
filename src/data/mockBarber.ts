...
export function generateNearbyBarbers(userLat: number, userLon: number, count = 5) {
  const offsets = [
    { dlat:  0.008, dlon:  0.005 },  
    { dlat: -0.005, dlon:  0.010 },  
    { dlat:  0.012, dlon: -0.007 },  
    { dlat: -0.018, dlon: -0.004 },  
    { dlat:  0.022, dlon:  0.015 },  
  ];

  return offsets.slice(0, count).map((off, idx) => ({
    id: idx + 1,
    name: BARBER_NAMES[idx] ?? `Barbershop ${idx + 1}`,
    latitude:  userLat + off.dlat,
    longitude: userLon + off.dlon,
  }));
}